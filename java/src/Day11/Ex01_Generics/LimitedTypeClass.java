...
// (Integer, Dobuble) 등 숫자 타입 클래스만 사용가능하도록 제한
...
public class LimitedTypeClass {
	...
	public static void main(String[] args) {
		...
		D<C> d3 = new D<C>();
//		(에러) D<X> d4 = new D<X();
		// D 클래스 타입 매개변수를 A클래스로 제한하고 있기 때문에, 
		// X 클래스 타입 매개변수를 지정 불가
		// A,B,C 클래스 타입 매개변수 지정 가능 
...
//		d3.setT(new A());
		...
		// (에러 : 숫자 타입이 아니여서 발생) E<Boolean> e4 = new E<Boolean>();
		...
		// 그냥 쓴 정수는 (127) : int 타입
		// e3 객체에 지정한 타입은 Byte(byte) -> 1byte 이기 떄문에 강제 형 변환이 필요
		e3.setT( (byte) 127);
